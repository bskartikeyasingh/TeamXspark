    ...
    <div className="min-h-screen bg-[#020617] text-white">
      ...
      >
        ...
        <div className="mt-auto p-4">
          ...
          <div className="mt-4 flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.02] p-3">
            <div className="flex items-center gap-3 min-w-0">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-cyan-400 to-blue-600 text-xs font-bold text-slate-950">
                {user?.role === "admin" ? "ADM" : (user?.displayName?.[0] || "STU")}
              </div>

              <div className="min-w-0">
                <p className="truncate text-xs font-semibold text-white">
                  {user?.displayName || user?.name || (user?.role === "admin" ? "Campus Commander" : "Student")}
                </p>

                <p className="truncate text-[10px] text-slate-500">
                  {user?.email || (user?.role === "admin" ? "Admin Access" : "Student Portal")}
                </p>
              </div>
            </div>
